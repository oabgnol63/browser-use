...
class TestDOMSerializer:
	...
	async def test_dom_serializer_with_shadow_dom_and_iframes(self, browser_session, base_url):
		...
		print('\n[ANALYSIS] DOM Serializer Analysis:')
		...
		# 	print(f'   [OK] Element {idx} found in serialized text')
		...
		for idx in indices:
			...
			print(f'   [OK] Element {idx} found in selector map')
		...
		print('\n[SUMMARY] DOM Serializer Test Summary:')
		...
		print('\n[CLICK] Testing Click Functionality:')
		...
		async def click(index: int, element_description: str, browser_session: BrowserSession):
			...
			print(f'   [OK] {element_description} [{index}] clicked successfully')
			return result
		...
		print('\n[VALIDATION] Validating click counter...')
	...
	async def test_stacked_complex_scenarios(self, browser_session, base_url):
		...
		selector_map = browser_state_summary.dom_state.selector_map
		print(f'\n[STACKED] Stacked Test - Found {len(selector_map)} elements')
		...
		print('\n[CLICK] Testing Click Functionality Through Stacked Layers:')
		...
		print('\n[CROSS-ORIGIN] Cross-Origin Iframe Extraction:')
		print(f'   Found elements from {len(all_targets)} different CDP targets in full DOM tree')

		if len(all_targets) >= 2:
			print('   [OK] Multi-target iframe extraction IS WORKING!')
		...
		if cross_origin_iframe_tag:
			print(f'\n   [PIN] Found cross-origin iframe tag [{cross_origin_iframe_tag[0]}]')
...
if __name__ == '__main__':
	...
	async def main():
		...
		print(f'\n[SERVER] HTTP Server running at {base_url}')
